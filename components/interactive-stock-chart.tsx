...
export const InteractiveStockChart = React.memo(
	({
		...
		const totalChange = useMemo(() => {
			...
			const sumInitial = processedData.reduce(
				(sum, series) => sum + series.firstPrice,
				0,
			);
			const sumFinal = processedData.reduce(
				(sum, series) => sum + series.lastPrice,
				0,
			);
    ...
    `;
				...
				if (params.length > 1) {
					params.forEach((param, index) => {
						...
						const seriesIndex =
							param.seriesIndex !== undefined ? param.seriesIndex : 0;
						...
						const series = processedData.find(
      ...
      `;
		...
		const chartOptions = useMemo<EChartsOption>(() => {
			...
			const uniqueCurrencies = Array.from(
				new Set(processedData.map((s) => s.currency)),
			);
		...
		return (
			<T id="components.interactive_stock_chart.2">
					...
					<div className="p-3 bg-muted/10">
						<div className="flex items-center justify-between mb-2">
							...
							>
								...
								<Var>{Math.abs(totalChange.percent).toFixed(2)}</Var>%
							</Badge>
						</div>
									...
									>
										<div className="flex items-center gap-1.5 mb-1">
											<div
												...
												style={{ backgroundColor: series.color.line }}
											></div>
						...
						<Var>
							{news_results &&
								...
								news_results.some((group) => group.results.length > 0) && (
									<T id="components.interactive_stock_chart.0">
																...
																<div className="flex items-start gap-1.5">
																	<div className="flex-1 min-w-0">
																		<div className="flex items-center gap-1.5 mb-1">
																			...
																			>
																				{group.topic === "finance" ? (
																					<T id="components.interactive_stock_chart.3">
																						{"Finance"}
																					</T>
																				) : (
																					<T id="components.interactive_stock_chart.4">
																						{"News"}
																					</T>
																				)}
																			</Badge>
																		...
																		<div className="flex items-center justify-between mt-1.5">
																			<div className="flex items-center gap-1">
																				...
																				<span className="text-[9px] text-muted-foreground/80 truncate max-w-[100px]">
																					{new URL(news.url).hostname.replace(
																						"www.",
																						"",
																					)}
																				</span>
						...
						<Var>
							{news_results &&
								news_results.some(
									(group) =>
										group.topic === "financial" && group.results.length > 0,
								) && (
									<T id="components.interactive_stock_chart.1">
										<div className="mt-5 pt-4 border-t border-border/30">
											<div className="flex items-center justify-between mb-2.5">
												<div className="flex items-center gap-1.5">
