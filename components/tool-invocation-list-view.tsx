							...
							<AccordionItem value="details" className="border-none">
								<AccordionTrigger className="py-1 hover:no-underline">
									<span className="text-xs font-medium text-neutral-500 dark:text-neutral-400 hover:text-red-500 dark:hover:text-red-400">
										{isExpanded ? (
											<T id="components.tool_invocation_list_view.0">
												{"Hide details"}
											</T>
										) : (
											<T id="components.tool_invocation_list_view.1">
												{"Show details"}
											</T>
										)}
									</span>
								...
								<AccordionContent>
									{video.timestamps && video.timestamps.length > 0 && (
										<T id="components.tool_invocation_list_view.2">
									...
									{video.captions && (
										<T id="components.tool_invocation_list_view.3">
...
function CollapsibleSection({
	...
	return (
		<div className="group rounded-lg border border-neutral-200 dark:border-neutral-800 overflow-hidden transition-all duration-200 hover:shadow-xs">
				...
				<div className="flex items-center gap-3">
					...
					<div>
						...
						<p className="text-xs text-neutral-500">
							{status === "running" ? (
								<T id="components.tool_invocation_list_view.4">
									{"Running..."}
								</T>
							) : (
								<T id="components.tool_invocation_list_view.5">{"Completed"}</T>
							)}
						</p>
					</div>
				</div>
			...
			{isExpanded && (
				<T id="components.tool_invocation_list_view.7">
					<div className="border-t border-neutral-200 dark:border-neutral-800">
						<div className="flex items-center border-b border-neutral-200 dark:border-neutral-800">
							...
							<Var>
								{output && (
									<T id="components.tool_invocation_list_view.6">
										<button
						...
						<div className="max-h-[500px] overflow-auto p-4 bg-neutral-50 dark:bg-neutral-900">
							<pre className="text-sm whitespace-pre-wrap overflow-x-auto">
								<Var>{activeTab === "code" ? code : output}</Var>
							</pre>
						</div>
					</div>
				</T>
...
const ToolInvocationListView = memo(
	({
		...
		const renderToolInvocation = useCallback(
			(toolInvocation: ToolInvocation, index: number) => {
				...
				if (toolInvocation.toolName === "find_place") {
					...
					return (
						<T id="components.tool_invocation_list_view.8">
							<Card className="w-full my-4 overflow-hidden bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-800">
								<div className="relative w-full h-[60vh]">
									<div className="absolute top-4 left-4 z-10 flex gap-2">
										...
										>
											<Var>{features.length}</Var> Locations Found
										</Badge>
									</div>
				...
				if (toolInvocation.toolName === "youtube_search") {
					...
					if (filteredVideos.length === 0) {
						return (
							<T id="components.tool_invocation_list_view.10">
									...
									<div className="flex flex-col items-center gap-3 py-6">
										...
										<div className="text-center">
											...
											<p className="text-sm text-neutral-500 dark:text-neutral-400">
												The videos found don&apos;t contain any timestamps or
												transcripts.
											</p>
										</div>
									</div>
					...
					return (
						<T id="components.tool_invocation_list_view.11">
											...
											<div className="flex items-center gap-3">
												...
												<div>
													...
													<div className="flex items-center gap-2 mt-0.5">
														...
														>
															<Var>{filteredVideos.length}</Var> videos with
															content
														</Badge>
													</div>
												</div>
				...
				if (toolInvocation.toolName === "text_search") {
					if (!result) {
						return (
							<T id="components.tool_invocation_list_view.13">
								<div className="flex items-center justify-between w-full">
									<div className="flex items-center gap-2">
										<MapPin className="h-5 w-5 text-neutral-700 dark:text-neutral-300 animate-pulse" />
										<span className="text-neutral-700 dark:text-neutral-300 text-lg">
				...
				if (toolInvocation.toolName === "get_weather_data") {
					if (!result) {
						return (
							<T id="components.tool_invocation_list_view.14">
								<div className="flex items-center justify-between w-full">
									<div className="flex items-center gap-2">
							...
							{result?.chart && (
								<div className="w-full">
									<InteractiveStockChart
										...
										currency_symbols={
											args.currency_symbols ||
											args.stock_symbols.map(() => "USD")
										}
				...
				if (toolInvocation.toolName === "retrieve") {
					...
					if (
						result.error ||
						(result.results && result.results[0] && result.results[0].error)
					) {
						const errorMessage =
							result.error ||
							(result.results && result.results[0] && result.results[0].error);
						return (
							<T id="components.tool_invocation_list_view.15">
					...
					if (!result.results || result.results.length === 0) {
						return (
							<T id="components.tool_invocation_list_view.16">
					...
					return (
						<T id="components.tool_invocation_list_view.19">
											...
											<div className="flex flex-wrap items-center gap-2 mt-2">
												...
												<Var>
													{result.response_time && (
														<T id="components.tool_invocation_list_view.17">
															<Badge
																variant="secondary"
															...
															>
																...
																<Var>{result.response_time.toFixed(1)}</Var>s
															</Badge>
														</T>
													)}
												</Var>
										...
										<div className="flex items-center gap-2">
											...
											<Var>
												{result.results.length > 1 && (
													<T id="components.tool_invocation_list_view.18">
														<Badge
															variant="secondary"
														...
														>
															...
															<Var>{result.results.length}</Var> pages
														</Badge>
													</T>
												)}
											</Var>
												...
												>
													<AccordionTrigger className="group px-4 py-3 text-xs font-medium text-neutral-700 dark:text-neutral-300 hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition-colors no-underline! rounded-t-none! data-[state=open]:rounded-b-none! data-[state=open]:bg-neutral-50 dark:data-[state=open]:bg-neutral-800/50 [&>svg]:h-4 [&>svg]:w-4 [&>svg]:text-neutral-500 [&>svg]:transition-transform [&>svg]:duration-200">
														<div className="flex items-center gap-2">
															...
															<span>
																{index === 0 ? (
																	<T id="components.tool_invocation_list_view.20">
																		{"View full content"}
																	</T>
																) : (
				...
				if (toolInvocation.toolName === "track_flight") {
					if (!result) {
						return (
							<T id="components.tool_invocation_list_view.21">
								<div className="flex items-center justify-between w-full">
									<div className="flex items-center gap-2">
					...
					if (result.error) {
						return (
							<T id="components.tool_invocation_list_view.22">
								<div className="text-red-500 dark:text-red-400">
									Error tracking flight: <Var>{result.error}</Var>
								</div>
							</T>
						);
					}
				...
				if (toolInvocation.toolName === "datetime") {
					if (!result) {
						return (
							<T id="components.tool_invocation_list_view.23">
					...
					return (
						<T id="components.tool_invocation_list_view.24">
				...
				if (toolInvocation.toolName === "mcp_search") {
					...
					return (
						<T id="components.tool_invocation_list_view.25">
		...
		const TranslationTool: React.FC<{
			...
			return (
				<T id="components.tool_invocation_list_view.26">
								...
								<div>
									<p className="text-sm sm:text-base text-neutral-600 dark:text-neutral-400 leading-relaxed">
										...
										<span className="font-medium text-neutral-900 dark:text-neutral-100">
											<Var>{toolInvocation.args.text}</Var>
										</span>{" "}
										translates from{" "}
										<span className="font-medium text-neutral-900 dark:text-neutral-100">
											<Var>{result.detectedLanguage}</Var>
										</span>{" "}
										to{" "}
										<span className="font-medium text-primary">
											<Var>{result.translatedText}</Var>
										</span>
									</p>
								</div>
