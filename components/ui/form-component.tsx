...
const GeminiIcon = ({ className }: { className?: string }) => (
	<T id="components.ui.form_component.0">
		<svg
			height="1em"
													...
													<div className="flex items-center gap-1 mt-0.5">
														{(model.vision || model.reasoning || model.pdf) && (
															<div className="flex gap-1">
																{model.vision && (
																	<T id="components.ui.form_component.2">
																		<div
																			className={cn(
																...
																{model.reasoning && (
																	<T id="components.ui.form_component.3">
																		<div
																			className={cn(
																				"flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[9px] font-medium",
																...
																{model.pdf && (
																	<T id="components.ui.form_component.4">
																		<div
																			className={cn(
																				"flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[9px] font-medium",
...
const AttachmentPreview: React.FC<{
	...
	const isUploadingAttachment = (
		attachment: Attachment | UploadingAttachment,
	): attachment is UploadingAttachment => {
		...
		>
			...
			<div className="grow min-w-0">
				...
				<p className="text-[10px] text-neutral-500 dark:text-neutral-400">
					{isUploadingAttachment(attachment) ? (
						<T id="components.ui.form_component.5">{"Uploading..."}</T>
					) : (
...
const SwitchNotification: React.FC<SwitchNotificationProps> = ({
	...
	const useModelColor =
		notificationType === "model" && modelColor !== "default";
							...
							return (
								<motion.div
									...
									animate={{
										...
										marginRight:
											showItem && isLastItem && isExpanded ? "2px" : 0,
									}}
									transition={{
										duration: 0.15,
										ease: "easeInOut",
		...
		<div className={cn("flex flex-col w-full")}>
			<TooltipProvider>
				...
				>
					<AnimatePresence>
						{isDragging && (
							<T id="components.ui.form_component.6">
								<motion.div
									initial={{ opacity: 0 }}
									animate={{ opacity: 1 }}
									exit={{ opacity: 0 }}
						...
						<div className="relative rounded-lg bg-neutral-100 dark:bg-neutral-900">
							...
							>
								...
								>
									...
									>
										{!isMobile ? (
											<T id="components.ui.form_component.7">
											...
											<T id="components.ui.form_component.8">
												<button
													onClick={(e) => {
														e.preventDefault();
														e.stopPropagation();
								...
								>
									{hasVisionSupport(selectedModel) &&
										!(isMobile && isGroupSelectorExpanded) &&
										(!isMobile ? (
											<T id="components.ui.form_component.11">
												<Tooltip delayDuration={300}>
													<TooltipTrigger asChild>
														<Button
															...
															<span className="text-[10px] text-neutral-300 dark:text-neutral-600 leading-tight">
																<Var>
																	{supportsPdfAttachments(selectedModel) ? (
																		<T id="components.ui.form_component.9">
																			{"Upload an image or PDF document"}
																		</T>
																	) : (
																		<T id="components.ui.form_component.10">
																			{"Upload an image"}
																		</T>
																	)}
																</Var>
									...
									{isProcessing ? (
										!isMobile ? (
											<T id="components.ui.form_component.12">
												<Tooltip delayDuration={300}>
													<TooltipTrigger asChild>
														<Button
										...
										<T id="components.ui.form_component.13">
											<Tooltip delayDuration={300}>
												<TooltipTrigger asChild>
													<Button
														className="rounded-full p-1.5 h-8 w-8"
